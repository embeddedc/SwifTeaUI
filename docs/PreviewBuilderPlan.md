...
2. **CLI/Neovim** – Expose `swift run PreviewDemo --list` and `--preview Notebook:Focused Editor`. CLI command enumerates `PreviewCatalog` and prints frames or streams to watchman.
...
3. Gradually retrofit Notebook/TaskRunner modules to declare preview providers and feed them into snapshot tests/Xcode integration later.
