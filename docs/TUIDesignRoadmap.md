...
| **Scrollable Viewports** | Keep text editors and long content bounded via scroll containers that mirror SwiftUI’s `ScrollView`. | Planned | Define `ScrollView(_ axis: Axis, viewport: Int, offset: Binding<Int>)` that renders children once, clamps to the requested line/window size, and slices strings with ANSI-aware helpers; add key-event mapping (`↑/↓/PageUp/PageDown`) plus programmatic `ensureVisible(line:)` hooks so `TextEditor` can auto-scroll the caret; surface optional indicators (e.g., `↑↓`) and modifiers like `.scrollDisabled(_:)`; wire demos so editable areas stay fixed height instead of growing indefinitely. |
