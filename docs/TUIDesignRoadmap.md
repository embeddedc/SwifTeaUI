...
| **Table Layout Component** | Provide a declarative table/grid view with column definitions, headers, and alignment options. | Planned | Introduce a `Table` view plus `TableColumn` descriptors that pre-measure column widths, pad cells consistently, support optional headers/row separators, and expose styling hooks for Mint-style dashboards. |
