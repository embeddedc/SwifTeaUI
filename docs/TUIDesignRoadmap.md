...
| **Scrollable Viewports** | Keep text editors and long content bounded via scroll containers that mirror SwiftUI’s `ScrollView`. | Prototype | `ScrollView(.vertical, viewport:offset:)` now renders children once, clamps to the requested line count, and feeds a binding so reducers can scroll via key events (see Notebook demo’s body editor). Next up: add horizontal slicing, optional indicators, and helper APIs that auto-scroll caret positions or expose `.scrollDisabled(_:)`. |
